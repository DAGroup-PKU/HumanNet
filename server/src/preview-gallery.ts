...
const tpvEntries = TPV_CLIPS.map(([file, width, height], index): PreviewManifestEntry => ({
  id: `tpv-preview-${String(index + 1).padStart(2, "0")}`,
  localPath: `media/curation/video/tpv/${file}`,
  ossKey: `preview/tpv/${file}`,
  perspective: "exo",
...
const fpvEntries = FPV_CLIPS.map(([file, task, caption], index): PreviewManifestEntry => ({
  id: `fpv-preview-${String(index + 1).padStart(2, "0")}`,
  localPath: `media/curation/video/fpv/${file}`,
  ossKey: `preview/fpv/${file}`,
  perspective: "ego",
  task,
  caption,
  width: 1280,
...
export const PREVIEW_GALLERY_MANIFEST = [
  ...tpvEntries,
  ...fpvEntries,
] satisfies PreviewManifestEntry[];

export const PREVIEW_GALLERY = PREVIEW_GALLERY_MANIFEST.map(
  ({ id, ossKey, perspective, task, caption, width, height }) => ({
    id,
    src: { kind: "oss" as const, key: ossKey },
    perspective,
    task,
    caption,
