...
def load(
    ...
    if os.path.isdir(model_id_or_path):
        ...
        assert len(checkpoint_files) > 0, f"No checkpoint found in `{checkpoint_dir}`"
        checkpoint_pt = sorted(checkpoint_files)[-1]  # 取最新的
        overwatch.info(f"Found checkpoint: {checkpoint_pt.name}")
    else:
